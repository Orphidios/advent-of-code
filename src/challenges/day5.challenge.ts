...
export class Day5Challenge extends Challenge<Day5Input> {
    ...
    protected runWithInput(input: Day5Input): void {
        const validUpdates = input.updates.filter(update => this.isValidUpdate(update, input.rules));
        const invalidUpdates = input.updates.filter(update => !this.isValidUpdate(update, input.rules));

        const middlePagesSumValid = validUpdates.reduce((sum, update) => sum + this.getMiddlePage(update), 0);
        console.log('Sum of middle pages (valid updates):', middlePagesSumValid);

        const reorderedUpdates = invalidUpdates.map(update => this.reorderUpdate(update, input.rules));
        const middlePagesSumInvalid = reorderedUpdates.reduce((sum, update) => sum + this.getMiddlePage(update), 0);
        console.log('Sum of middle pages (reordered updates):', middlePagesSumInvalid);
    }
