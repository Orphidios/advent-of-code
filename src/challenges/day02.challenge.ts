...
export class Day2Challenge extends Challenge<Day2Input> {
  ...
  protected countValidReports(
    reports: Day2Input,
    withDampener: boolean = false,
  ): number {
    if (withDampener) {
      return reports.filter((report) => this.isReportValidWithDampener(report))
        .length;
    }
    return reports.filter((report) => this.analyseReport(report).isValid)
      .length;
  }
  ...
  protected analyseReport(report: number[]): {
    isValid: boolean;
    ...
    for (let i = 0; i < report.length - 1; i++) {
      const isSameDirection =
        this.getDirection(report[i], report[i + 1]) === baseDirection;
